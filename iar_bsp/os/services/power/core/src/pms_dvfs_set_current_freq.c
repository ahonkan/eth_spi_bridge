...
*   DESCRIPTION
*
*       Contains all functionality for setting the current frequency
*
*   DATA STRUCTURES
*
*       None
...
*       frequency (given by freq_id) and current voltage id point. If
*       such OP is successfully identified, the transition to that point
*       is attempted.
*
*   INPUT
*
...
*       PM_INVALID_FREQ_ID  Provided frequency ID is not valid
*       PM_INVALID_OP_ID    Provided frequency with current voltage
...
STATUS NU_PM_Set_Current_Freq(UINT8 freq_id)
{
    ...
    if (pm_status == NU_SUCCESS)
    {
        ...
        {
            /* Read the voltage id */
            volt_id = PM_DVFS_OP_Array[PM_DVFS_Current_OP] -> pm_volt_id;
