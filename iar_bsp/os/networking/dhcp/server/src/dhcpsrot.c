...
* DESCRIPTION                                                          
*                                                                      
*     Add the stated router IP address to the specified configuration control block 
*     or the specific binding.
*
* INPUTS                                                               
*                                                                      
*     DHCPS_CONFIG_CB *
...
*                                                                      
**********************************************************************************/
STATUS DHCPS_Add_Router (DHCPS_CONFIG_CB *config_cb, const struct id_struct *client_ip_addr, 
                             struct id_struct *router_address)
{
    ...
    router_ip = IP_ADDR(router_address->is_ip_addrs);

    /* If this is not a class A, B, or C address, or if it is 0 then it is not 
       a valid IP address. */
    if (   (!IP_CLASSA_ADDR(router_ip) 
         && !IP_CLASSB_ADDR(router_ip) 
         && !IP_CLASSC_ADDR(router_ip))
         || router_ip == 0)
    {
        ret_status = NU_INVALID_PARM;
    }

    /* First, we must determine if this router entry is for a particular IP address (static IP).  
        This can be determined if a client IP address was passed in.  If so, then the router 
    ...
    if (client_ip_addr != NU_NULL)
    {
        /* Perform any memory block manipulation that may need to be done to make room for
            the router IP address that is being added. */
        options_block = DHCPS_Add_Option_To_Memory_Block(config_cb, client_ip_addr, ROUTER, &option_buffer);    
        ...
        if (options_block != NU_NULL)
        {
            ...
            /* The router can now be added to the options block. First, we must ensure that 
                the option has been previously added.  If not, we can just add the option tag and
                data length now. */
            if (*options_block == DHCP_END)
            {
                /* Store the router into the memory block in the format that it will be
                   provided to the client. */

                /* Option Tag */
                *options_block++ = ROUTER;
                ...
                *options_block++ = IP_ADDR_LEN;
                ...
                for (i = 0; i < IP_ADDR_LEN; i++)
                    *options_block++ = router_address->is_ip_addrs[i];
            ...
            {
                /* We must move to the end of the option and append our router entry to the 
            ...
            /* An invalid parameter was passed into the function.  The router can not be added. */
            ret_status = NU_INVALID_PARM;
    }       
    ...
    else
    {
        ...
        ret_status = DHCPS_Add_Entry_To_Array(&config_cb->router[0], router_address, 
                                                DHCPS_MAX_ROUTERS);
...
*                                                                      
**********************************************************************************/
STATUS DHCPS_Delete_Router (DHCPS_CONFIG_CB *config_cb, const struct id_struct *client_ip_addr, 
                                  const struct id_struct *router_addr)
{
    STATUS                      ret_status;
    ...
    if (client_ip_addr != NU_NULL)
    {
        /* The router must be removed from the option memory block and the remaining contents
            of the block adjusted to compensate for the removal of an entry. */
        ret_status = DHCPS_Remove_Option_From_Memory_Block(config_cb, client_ip_addr, ROUTER, router_addr);
    }
    ...
    else
    {
        ...
        /* Remove the router IP address from the array. */
        ret_status = DHCPS_Remove_Entry_From_Array(&config_cb->router[0], 
                                                router_addr, DHCPS_MAX_ROUTERS);        
...
* DESCRIPTION                                                                
*                                                                            
*    Return the router IP addresses of the specified configuration control block or
...
*                                                                            
******************************************************************************/
INT DHCPS_Get_Router(const DHCPS_CONFIG_CB *config_cb, const struct id_struct *client_ip_addr, 
                     UINT8 *router_buffer, INT buffer_size)
{
    UINT8                       *option_ptr,
                                option_data_len,
    ...
    if ((config_cb == NU_NULL) || (router_buffer == NU_NULL))
    {
    ...
    {
        /* Search through the static IP entries to find the router entries. */
        option_ptr = DHCPS_Get_Option_From_Memory_Block(config_cb, client_ip_addr, ROUTER);
        ...
        {
            /* The router options have been found for the desired IP address. */

            /* Protect the global data structures. */
            NU_Obtain_Semaphore(&DHCPS_Semaphore, NU_SUSPEND);

            /* Increment the option pointer to point at the length of the router option. */
            ...
            else
            {
                ...
                /* Copy the router options into the buffer that has been provided by the application. */
                memcpy(router_buffer, option_ptr, option_data_len);
    ...
    else
    {
        ...
        /* The desired option might be inside the configuration control block.  Increment through the 
            router address array and copy then into provided buffer. */
        for (i = 0; i < DHCPS_MAX_ROUTERS; i++)
        {
            ...
            else if (config_cb->router[i].is_ip_addrs[0] == 0)
            {
            ...
            else
            {
                ...
                memcpy(router_buffer, config_cb->router[i].is_ip_addrs, IP_ADDR_LEN);

                /* Increment a counting variable. */
                entries_written_to_buff++;

                /* Increment the buffer pointer */
                router_buffer += IP_ADDR_LEN;
            }    
        }    
        /* Unprotect the global data structures. */
        NU_Release_Semaphore(&DHCPS_Semaphore);
