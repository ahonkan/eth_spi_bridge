...
*   DESCRIPTION
*
*       This file contains the FTP Client implementation of STAT
*
*   DATA STRUCTURES
*
*       None
...
*   DESCRIPTION
*
*       This function provides a primitive to send an FTP STAT (status)
*       message to the server. Parameters include a pointer to a valid FTP
*       client structure, a pointer to a character string containing the path
*       to the object whose status is requested, a pointer to a character
*       buffer to store the status information, and the size of the buffer.
*
*   INPUTS
*
*       client                  pointer to valid FTP client structure
*       path                    pointer to character string containing path
*                               to object whose status is desired
*       buffer                  pointer to the storage location for status
*                               information
...
*
******************************************************************************/
INT FCP_Client_STAT(FTP_CLIENT *client, CHAR *path, CHAR *buffer, INT buffsize)
{
    INT     status,
            index,
            bytes_received;
    ...
    if ( (client == NU_NULL) || (path == NU_NULL) || (buffer == NU_NULL) ||
         (buffsize <= 0) )
