...
*   DESCRIPTION
*
*       This file contains the FTP Client implementation of PASS.
*
*   DATA STRUCTURES
*
*       None
...
*   DESCRIPTION
*
*       This function provides a primitive to send an FTP PASS message to the
*       server. Parameters include a pointer to a valid FTP client structure
*       and a pointer to a character string containing the password to send to
*       the server.
*
*   INPUTS
*
...
INT FCP_Client_PASS(FTP_CLIENT *client, CHAR *password)
{
    ...
    if (password)
    {
        ...
        /* Accumulate password into buffer */
        while (*password)
        {
            buffer[index] = (UINT8)*password;
            password++;
            index++;
    ...
    {
        client->last_error = FTP_INVALID_PASSWORD;
    }
