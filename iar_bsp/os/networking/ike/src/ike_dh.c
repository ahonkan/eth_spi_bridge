/*************************************************************************
*
*               Copyright 2005 Mentor Graphics Corporation
*                         All Rights Reserved.
*
* THIS WORK CONTAINS TRADE SECRET AND PROPRIETARY INFORMATION WHICH IS THE
* PROPERTY OF MENTOR GRAPHICS CORPORATION OR ITS LICENSORS AND IS SUBJECT
* TO LICENSE TERMS.
*
**************************************************************************

**************************************************************************
*
* FILE NAME
*
*       ike_dh.c
*
* COMPONENT
*
*       IKE - Diffie-Hellman Groups
*
* DESCRIPTION
*
*       This file contains data and functions used in the IKE
*       Diffie-Hellman Key Exchange.
*
* DATA STRUCTURES
*
*       IKE_Oakley_Prime_MODP768    The First Oakley group.
*       IKE_Oakley_Prime_MODP1024   The Second Oakley group.
*       IKE_Oakley_Prime_MODP1536   Fifth Oakley group.
*       IKE_Oakley_Prime_MODP2048   2048-bit Oakley group.
*       IKE_Oakley_Prime_MODP3072   3072-bit Oakley group.
*       IKE_Oakley_Prime_MODP4096   4096-bit Oakley group.
*       IKE_Oakley_Prime_MODP6144   6144-bit Oakley group.
*       IKE_Oakley_Prime_MODP8192   8192-bit Oakley group.
*       IKE_Oakley_Prime_MODP1024_160_POS   22nd Oakley group.
*       IKE_Oakley_Prime_MODP1024_160_POS_G 22nd Oakley group generator.
*       IKE_Oakley_Prime_MODP2048_224_POS   23rd Oakley group.
*       IKE_Oakley_Prime_MODP2048_224_POS_G 23rd Oakley group generator.
*       IKE_Oakley_Prime_MODP2048_256_POS   24th Oakley group.
*       IKE_Oakley_Prime_MODP2048_256_POS_G 24th Oakley group generator.
*
* FUNCTIONS
*
*       IKE_Oakley_Group_Prime
*       IKE_Oakley_Group_Length
*       IKE_Get_Oakley_Group
*
* DEPENDENCIES
*
*       nu_net.h
*       ike_cfg.h
*       ike_doi.h
*       ike_dh.h
*
*************************************************************************/
#include "networking/nu_net.h"
#include "networking/ike_cfg.h"
#include "networking/ike_doi.h"
#include "networking/ike_dh.h"

/* The First Oakley Group prime number (RFC 2409). */
UINT8 IKE_Oakley_Prime_MODP768[] =
{
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xC9, 0x0F, 0xDA, 0xA2, 0x21, 0x68, 0xC2, 0x34,
    0xC4, 0xC6, 0x62, 0x8B, 0x80, 0xDC, 0x1C, 0xD1,
    0x29, 0x02, 0x4E, 0x08, 0x8A, 0x67, 0xCC, 0x74,
    0x02, 0x0B, 0xBE, 0xA6, 0x3B, 0x13, 0x9B, 0x22,
    0x51, 0x4A, 0x08, 0x79, 0x8E, 0x34, 0x04, 0xDD,
    0xEF, 0x95, 0x19, 0xB3, 0xCD, 0x3A, 0x43, 0x1B,
    0x30, 0x2B, 0x0A, 0x6D, 0xF2, 0x5F, 0x14, 0x37,
    0x4F, 0xE1, 0x35, 0x6D, 0x6D, 0x51, 0xC2, 0x45,
    0xE4, 0x85, 0xB5, 0x76, 0x62, 0x5E, 0x7E, 0xC6,
    0xF4, 0x4C, 0x42, 0xE9, 0xA6, 0x3A, 0x36, 0x20,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
};

#if (IKE_INCLUDE_MODP_1024 == NU_TRUE)
/* The Second Oakley Group prime number (RFC 2409). */
UINT8 IKE_Oakley_Prime_MODP1024[] =
{
    
};
#endif

#if (IKE_INCLUDE_MODP_1536 == NU_TRUE)
/* Oakley Group defined in RFC 3526. */
UINT8 IKE_Oakley_Prime_MODP1536[] =
{
    
};
#endif

#if (IKE_INCLUDE_MODP_2048 == NU_TRUE)
/* Oakley Group defined in RFC 3526. */
UINT8 IKE_Oakley_Prime_MODP2048[] =
{
    
};
#endif

#if (IKE_INCLUDE_MODP_3072 == NU_TRUE)
/* Oakley Group defined in RFC 3526. */
UINT8 IKE_Oakley_Prime_MODP3072[] =
{
    
};
#endif

#if (IKE_INCLUDE_MODP_4096 == NU_TRUE)
/* Oakley Group defined in RFC 3526. */
UINT8 IKE_Oakley_Prime_MODP4096[] =
{
    
};
#endif

#if (IKE_INCLUDE_MODP_6144 == NU_TRUE)
/* Oakley Group defined in RFC 3526. */
UINT8 IKE_Oakley_Prime_MODP6144[] =
{
    
};
#endif

#if (IKE_INCLUDE_MODP_8192 == NU_TRUE)
/* Oakley Group defined in RFC 3526. */
UINT8 IKE_Oakley_Prime_MODP8192[] =
{
    
};
#endif

#if (IKE_INCLUDE_MODP_1024_160_POS == NU_TRUE)
/* Oakley Group Prime and Generator defined in RFC 5114. */
UINT8 IKE_Oakley_Prime_MODP1024_160_POS[] =
{
    
};

UINT8 IKE_Oakley_Prime_MODP1024_160_POS_G[] =
{
    
};
#endif

#if (IKE_INCLUDE_MODP_2048_224_POS == NU_TRUE)
/* Oakley Group Prime and Generator defined in RFC 5114. */
UINT8 IKE_Oakley_Prime_MODP2048_224_POS[] =
{
    
};

UINT8 IKE_Oakley_Prime_MODP2048_224_POS_G[] =
{
    
};
#endif

#if (IKE_INCLUDE_MODP_2048_256_POS == NU_TRUE)
/* Oakley Group Prime and Generator defined in RFC 5114. */
UINT8 IKE_Oakley_Prime_MODP2048_256_POS[] =
{
    
};

UINT8 IKE_Oakley_Prime_MODP2048_256_POS_G[] =
{
    
};
#endif


/* An array of the above Oakley Groups. */
IKE_OAKLEY_GROUP_INFO IKE_Oakley_Groups[] =
{
    { IKE_Oakley_Prime_MODP768,    96,  IKE_GROUP_MODP_768,
      IKE_DH_GENERATOR_2,     NU_NULL,  0, 0 },
#if (IKE_INCLUDE_MODP_1024 == NU_TRUE)
    { IKE_Oakley_Prime_MODP1024,  128,  IKE_GROUP_MODP_1024,
      IKE_DH_GENERATOR_2,     NU_NULL,  0, 0 },
#endif
#if (IKE_INCLUDE_MODP_1536 == NU_TRUE)
    { IKE_Oakley_Prime_MODP1536,  192,  IKE_GROUP_MODP_1536,
      IKE_DH_GENERATOR_2,     NU_NULL,  0, 0 },
#endif
#if (IKE_INCLUDE_MODP_2048 == NU_TRUE)
    { IKE_Oakley_Prime_MODP2048,  256,  IKE_GROUP_MODP_2048,
      IKE_DH_GENERATOR_2,     NU_NULL,  0, 0 },
#endif
#if (IKE_INCLUDE_MODP_3072 == NU_TRUE)
    { IKE_Oakley_Prime_MODP3072,  384,  IKE_GROUP_MODP_3072,
      IKE_DH_GENERATOR_2,     NU_NULL,  0, 0 },
#endif
#if (IKE_INCLUDE_MODP_4096 == NU_TRUE)
    { IKE_Oakley_Prime_MODP4096,  512,  IKE_GROUP_MODP_4096,
      IKE_DH_GENERATOR_2,     NU_NULL,  0, 0 },
#endif
#if (IKE_INCLUDE_MODP_6144 == NU_TRUE)
    { IKE_Oakley_Prime_MODP6144,  768,  IKE_GROUP_MODP_6144,
      IKE_DH_GENERATOR_2,     NU_NULL,  0, 0 },
#endif
#if (IKE_INCLUDE_MODP_8192 == NU_TRUE)
    { IKE_Oakley_Prime_MODP8192, 1024,  IKE_GROUP_MODP_8192,
      IKE_DH_GENERATOR_2,     NU_NULL,  0, 0 },
#endif
#if (IKE_INCLUDE_MODP_1024_160_POS == NU_TRUE)
    { IKE_Oakley_Prime_MODP1024_160_POS, 128,  IKE_GROUP_MODP_1024_160_POS,
      IKE_DH_GENERATOR_POS, IKE_Oakley_Prime_MODP1024_160_POS_G, 128, 20 },
#endif
#if (IKE_INCLUDE_MODP_2048_224_POS == NU_TRUE)
    { IKE_Oakley_Prime_MODP2048_224_POS, 256,  IKE_GROUP_MODP_2048_224_POS,
      IKE_DH_GENERATOR_POS, IKE_Oakley_Prime_MODP2048_224_POS_G, 256, 28 },
#endif
#if (IKE_INCLUDE_MODP_2048_256_POS == NU_TRUE)
    { IKE_Oakley_Prime_MODP2048_256_POS, 256,  IKE_GROUP_MODP_2048_256_POS,
      IKE_DH_GENERATOR_POS, IKE_Oakley_Prime_MODP2048_256_POS_G, 256, 32 },
#endif
};

/*************************************************************************
*
* FUNCTION
*
*       IKE_Oakley_Group_Prime
*
* DESCRIPTION
*
*       This function returns the Oakley Group prime number based
*       on the IKE Group Description.
*
* INPUTS
*
*       type                    Group description of the Oakley Group.
*
* OUTPUTS
*
*       Prime number or NU_NULL on error.
*
*************************************************************************/
UINT8 *IKE_Oakley_Group_Prime(UINT16 type)
{
    UINT8             i;

    /* Loop for all available Oakley Groups. */
    for(i = 0; i < IKE_TOTAL_OAKLEY_GROUPS; i++)
    {
        /* If the specified group is found. */
        if(IKE_Oakley_Groups[i].ike_group_desc == type)
        {
            return (IKE_Oakley_Groups[i].ike_prime);
        }
    }

    /* Group not found. */
    return (NU_NULL);

} /* IKE_Oakley_Group_Prime */

/*************************************************************************
*
* FUNCTION
*
*       IKE_Oakley_Group_Length
*
* DESCRIPTION
*
*       This function returns the length of the specified
*       Oakley Group's prime number, in bytes.
*
* INPUTS
*
*       type                    Group description of the Oakley Group.
*
* OUTPUTS
*
*       Prime number length or 0 on error.
*
*************************************************************************/
UINT16 IKE_Oakley_Group_Length(UINT16 type)
{
    UINT8             i;

    /* Loop for all available Oakley Groups. */
    for(i = 0; i < IKE_TOTAL_OAKLEY_GROUPS; i++)
    {
        /* If the specified group is found. */
        if(IKE_Oakley_Groups[i].ike_group_desc == type)
        {
            return (IKE_Oakley_Groups[i].ike_prime_len);
        }
    }

    /* Group not found. */
    return (0);

} /* IKE_Oakley_Group_Length */

/*************************************************************************
*
* FUNCTION
*
*       IKE_Get_Oakley_Group
*
* DESCRIPTION
*
*       This function returns information regarding the
*       specified Oakley Group or NULL if the group is not found.
*
* INPUTS
*
*       type                    Group description of the Oakley Group.
*
* OUTPUTS
*
*       Pointer to the informational structure or NU_NULL if not found.
*
*************************************************************************/
IKE_OAKLEY_GROUP_INFO *IKE_Get_Oakley_Group(UINT16 type)
{
    UINT8             i;

    /* Loop for all available Oakley Groups. */
    for(i = 0; i < IKE_TOTAL_OAKLEY_GROUPS; i++)
    {
        /* If the specified group is found. */
        if(IKE_Oakley_Groups[i].ike_group_desc == type)
        {
            return (&IKE_Oakley_Groups[i]);
        }
    }

    /* Group not found. */
    return (NU_NULL);

} /* IKE_Get_Oakley_Group */
