 ...
 */
static char * wpa_config_get_line(char *s, int size, int stream, int *line,
				  char **_pos)
{
	char *pos, *end, *sstart;

	while (os_fgets(s, size, stream)) {
		(*line)++;
		s[size - 1] = '\0';
		pos = s;
...
static struct wpa_ssid * wpa_config_read_network(int f, int *line, int id)
{
...
static struct wpa_config_blob * wpa_config_read_blob(int f, int *line,
						     const char *name)
{
	...
	while (wpa_config_get_line(buf, sizeof(buf), f, line, &pos)) {
		...
		len = os_strlen(pos);
		nencoded = (unsigned char *)os_realloc(encoded, encoded_len + len);
		if (nencoded == NULL) {
...
static int wpa_config_process_blob(struct wpa_config *config, int f,
				   int *line, char *bname)
{
	char *name_end;
	struct wpa_config_blob *blob;
...
struct wpa_config * wpa_config_read(const char *name)
{
	...
	f = os_file_open(name, PO_RDONLY, PS_IREAD);
	if (f < 0) {
		os_free(config);
		return NULL;
	}
...
static void write_str(int f, const char *field, struct wpa_ssid *ssid)
{
	...
	if (value == NULL)
		return;
	sprintf(str,"\t%s=%s\n", field, value);
	os_file_fprintf(f, str);
	os_free(value);
}


static void write_int(int f, const char *field, int value, int def)
{
	char str[64];
	if (value == def)
		return;
...
static void write_bssid(int f, struct wpa_ssid *ssid)
{
...
static void write_psk(int f, struct wpa_ssid *ssid)
{
...
static void write_proto(int f, struct wpa_ssid *ssid)
{
	char *value;
	char str[64];
...
static void write_key_mgmt(int f, struct wpa_ssid *ssid)
{
	char *value;
	char str[64];
...
static void write_pairwise(int f, struct wpa_ssid *ssid)
{
	char *value;
	char str[64];
...
static void write_group(int f, struct wpa_ssid *ssid)
{
	char *value;
	char str[64];
...
static void write_auth_alg(int f, struct wpa_ssid *ssid)
{
	char *value;
	char str[64];
	if (ssid->auth_alg == 0)
...
static void write_eap(int f, struct wpa_ssid *ssid)
{
	char *value;
	char str[64];
...
static void write_wep_key(int f, int idx, struct wpa_ssid *ssid)
{
	char field[20], *value;
	int res;
	char str[64];
...
static void wpa_config_write_network(int f, struct wpa_ssid *ssid)
{
	int i;
...
#ifndef CONFIG_NO_CONFIG_BLOBS
static int wpa_config_write_blob(int f, struct wpa_config_blob *blob)
{
...
static void wpa_config_write_global(int f, struct wpa_config *config)
{
	char str[64];
#ifdef CONFIG_CTRL_IFACE
	if (config->ctrl_interface) {
		sprintf(str, "ctrl_interface=%s\n", config->ctrl_interface);
		os_file_fprintf(f, str);
	}
	if (config->ctrl_interface_group) {
		sprintf(str, "ctrl_interface_group=%s\n",
			config->ctrl_interface_group);
		os_file_fprintf(f, str);
	}
#endif /* CONFIG_CTRL_IFACE */
	if (config->eapol_version != DEFAULT_EAPOL_VERSION) {
		sprintf(str, "eapol_version=%d\n", config->eapol_version);
		os_file_fprintf(f, str);
	}
	if (config->ap_scan != DEFAULT_AP_SCAN) {
		sprintf(str, "ap_scan=%d\n", config->ap_scan);
		os_file_fprintf(f, str);
	}
	if (config->fast_reauth != DEFAULT_FAST_REAUTH) {
		sprintf(str, "fast_reauth=%d\n", config->fast_reauth);
		os_file_fprintf(f, str);
	}
#ifdef EAP_TLS_OPENSSL
	if (config->opensc_engine_path) {
		sprintf(str, "opensc_engine_path=%s\n",
			config->opensc_engine_path);
		os_file_fprintf(f, str);
	}
	if (config->pkcs11_engine_path) {
		sprintf(str, "pkcs11_engine_path=%s\n",
			config->pkcs11_engine_path);
		os_file_fprintf(f, str);
	}
	if (config->pkcs11_module_path) {
		sprintf(str, "pkcs11_module_path=%s\n",
			config->pkcs11_module_path);
		os_file_fprintf(f, str);
	}
#endif /* EAP_TLS_OPENSSL */
	if (config->driver_param) {
		sprintf(str, "driver_param=%s\n", config->driver_param);
		os_file_fprintf(f, str);
	}
	if (config->dot11RSNAConfigPMKLifetime) {
		sprintf(str, "dot11RSNAConfigPMKLifetime=%d\n",
			config->dot11RSNAConfigPMKLifetime);
		os_file_fprintf(f, str);
	}
	if (config->dot11RSNAConfigPMKReauthThreshold) {
		sprintf(str, "dot11RSNAConfigPMKReauthThreshold=%d\n",
			config->dot11RSNAConfigPMKReauthThreshold);
		os_file_fprintf(f, str);
	}
	if (config->dot11RSNAConfigSATimeout) {
		sprintf(str, "dot11RSNAConfigSATimeout=%d\n",
			config->dot11RSNAConfigSATimeout);
		os_file_fprintf(f, str);
	}
	if (config->update_config) {
		sprintf(str, "update_config=%d\n", config->update_config);
		os_file_fprintf(f, str);
	}
#ifdef CONFIG_WPS
	...
	if (config->model_name){
		sprintf(str, "model_name=%s\n", config->model_name);
		os_file_fprintf(f, str);
	}
	if (config->model_number){
		sprintf(str, "model_number=%s\n", config->model_number);
		os_file_fprintf(f, str);
	}
	if (config->serial_number){
		sprintf(str, "serial_number=%s\n", config->serial_number);
		os_file_fprintf(f, str);
	}
	if (config->device_type){
		sprintf(str, "device_type=%s\n", config->device_type);
		os_file_fprintf(f, str);
	}
	if (WPA_GET_BE32(config->os_version)){
		sprintf(str, "os_version=%08x\n",
			WPA_GET_BE32(config->os_version));
		os_file_fprintf(f, str);
	}
	if (config->wps_cred_processing) {
		sprintf(str, "wps_cred_processing=%d\n",
			config->wps_cred_processing);
		os_file_fprintf(f, str);
	}
#endif /* CONFIG_WPS */
	if (config->country[0] && config->country[1]) {
		sprintf(str, "country=%c%c\n",
			config->country[0], config->country[1]);
		os_file_fprintf(f, str);
	}
}
...
int wpa_config_write(const char *name, struct wpa_config *config)
{
	...
	f = os_file_open(name, PO_WRONLY | PO_CREAT | PO_TRUNC, PS_IWRITE);
	if (f < 0) {
