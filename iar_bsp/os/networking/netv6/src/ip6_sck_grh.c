...
*       value previously set for the routing header on the socket.
*                                                                          
*   DATA STRUCTURES                                                          
*                      
*       None                
...
*       for the routing header on the socket.
*                                                                         
*   INPUTS                                                                
*                                                                         
*       socketd                 Specifies a socket descriptor
*       *rt_hdr                 A pointer to the ip6_rthdr to fill
*                               in with the routing header Sticky Option 
*                               data.
*       *optlen                 The length of the data in rt_hdr.
*
*   OUTPUTS                                                               
*                                                                         
...
*                                                                       
*************************************************************************/
STATUS IP6_Get_IPV6_RTHDR(INT socketd, struct ip6_rthdr *rt_hdr, INT *optlen)
{
    INT                 rthdr_len;
    ...
    if ( (tx_sticky_options) && (tx_sticky_options->tx_route_header) )
    {
        rthdr_len = 
            sizeof(struct ip6_rthdr0) + 
            (((struct ip6_rthdr*)tx_sticky_options->tx_route_header)->ip6r_len << 3);

        if (rthdr_len <= *optlen)
        {
            NU_BLOCK_COPY(rt_hdr, tx_sticky_options->tx_route_header,
