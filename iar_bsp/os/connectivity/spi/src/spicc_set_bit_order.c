...
* DESCRIPTION
*
*       This file contains the service that sets the bit order.
*
* DATA STRUCTURES
*
*       None
...
* FUNCTIONS
*
*       SPICC_Set_Bit_Order                 Sets bit order.
*
* DEPENDENCIES
*
...
* DESCRIPTION
*
*       This function sets the bit order for the specified slave of the
                            ...
                            UINT8       bit_order)
{
    ...
    if (status == NU_SUCCESS)
    {
        ...
        if (status == NU_SUCCESS)
        {
            /* Check bit order parameter for validity. */
    ...
    if (status == NU_SUCCESS)
    {
        ...
        /* Specify the desired bit order. */
        spi_config.spi_bit_order = bit_order;
        ...
        if (status == NU_SUCCESS)
        {
            ...
            if(config != NU_NULL)
            {
                /* Record the specified bit order in the associated transfer
                   attribute configuration record. */
                config->spi_bit_order = bit_order;
            }
            else
            {
