...
* FUNCTIONS
*
*       NU_USBH_MS_SCSI_Inquiry             Transfer INQUIRY command.
*       NU_USBH_MS_SCSI_Test_Unit_Ready     Transfer TEST UNIT READY
*                                           command.
*       NU_USBH_MS_SCSI_Read_Capacity       Transfer READ CAPACITY command.
*       NU_USBH_MS_SCSI_Request_Sense       Transfer REQUEST SENSE command.
*       NU_USBH_MS_SCSI_Read10              Transfer READ10 command.
*       NU_USBH_MS_SCSI_Write10             Transfer WRITE10 command.
*       NU_USBH_MS_SCSI_Request             General SCSI command transfer
*                                           request API.
*
* DEPENDENCIES
*
...
*
**************************************************************************/
#ifndef NU_USBH_MS_SCSI_IMP_C
#define NU_USBH_MS_SCSI_IMP_C
/* ==============  USB Include Files ==================================  */
...
*
**************************************************************************/
STATUS  NU_USBH_MS_SCSI_Inquiry(VOID *handle,
                    UINT8 command[NU_SCSI_CML_MAX],
                    VOID *buffer, UINT8 buf_len)
{
    STATUS  status;
   ...
   memset(command, NU_NULL, NU_SCSI_CML_INQUIRY);

   /* Set command code. */
   command[0] = NU_SCSI_CMD_INQUIRY;

   /* Set data length. */
   command[4] = buf_len;

   status = NU_USBH_MS_SCSI_Request(
                   handle,             /* Handle.                   */
                   command,            /* Pointer to command block. */
                   NU_SCSI_CML_INQUIRY,/* Length of CBWCB.          */
...
*
**************************************************************************/
STATUS  NU_USBH_MS_SCSI_Test_Unit_Ready(VOID *handle,
                                UINT8 command[NU_SCSI_CML_MAX])
{
    STATUS  status;
    ...
    memset(command, 0, NU_SCSI_CML_TST_UNT_RDY);

    /* Set command code. */
    command[0] = NU_SCSI_CMD_TST_UNT_RDY;

    status = NU_USBH_MS_SCSI_Request(
                    handle,             /* Handle.                   */
                    command,            /* Pointer to command block. */
                    /* Length of CBWCB. */
                    NU_SCSI_CML_TST_UNT_RDY,
                    0,                  /* Pointer to buffer.        */
                    0,                  /* Data length.              */
...
*
**************************************************************************/
STATUS  NU_USBH_MS_SCSI_Read_Capacity(VOID *handle,
                                UINT8 command[NU_SCSI_CML_MAX],
                                UINT8 buffer[NU_SCSI_DL_RD_CAP])
{
    STATUS  status;
    ...
    memset(command, 0, NU_SCSI_CML_RD_CAP);

    /* Set command code. */
    command[0] = NU_SCSI_CMD_RD_CAP;

    status = NU_USBH_MS_SCSI_Request(
                    handle,             /* Handle.                   */
                    command,            /* Pointer to command block. */
                    NU_SCSI_CML_RD_CAP, /* Length of CBWCB.          */
                    (VOID *)buffer,     /* Pointer to buffer.        */
                    NU_SCSI_DL_RD_CAP,  /* Data length.              */
                    USB_DIR_IN);        /* Direction.                */
...
*
**************************************************************************/
STATUS  NU_USBH_MS_SCSI_Request_Sense(VOID *handle,
                                UINT8 command[NU_SCSI_CML_MAX],
                                VOID *buffer, UINT8 buf_len)
{
    STATUS  status;
    ...
    memset(command, 0, NU_SCSI_CML_REQ_SNS);

    /* Set command code. */
    command[0] = NU_SCSI_CMD_REQ_SNS;
    ...
    status = NU_USBH_MS_SCSI_Request(
                    handle,             /* Handle.                   */
                    command,            /* Pointer to command block. */
                    NU_SCSI_CML_REQ_SNS,/* Length of CBWCB.          */
                    buffer,             /* Pointer to buffer.        */
                    buf_len,            /* Data length.              */
                    USB_DIR_IN);        /* Direction.                */
...
* DESCRIPTION
*
*       Read data by SCSI read command.
*
* INPUTS
*
*       handle                              Handle.
...
*
**************************************************************************/
STATUS  NU_USBH_MS_SCSI_Read10( VOID    *handle,
                                UINT8   command[NU_SCSI_CML_MAX],
                                UINT32  block_addr,
                                UINT32  block_num,
                                VOID    *buffer,
                                UINT32  buf_len )
{
    ...
    memset(command, 0, NU_SCSI_CML_READ10);

    /* Set command code. */
    command[0] = NU_SCSI_CMD_READ10;
    ...
    status = NU_USBH_MS_SCSI_Request(
                    handle,             /* Handle.                   */
                    command,            /* Pointer to command block. */
                    NU_SCSI_CML_READ10, /* Length of CBWCB.          */
                    buffer,             /* Pointer to buffer.        */
                    buf_len,            /* Data length.              */
                    USB_DIR_IN);        /* Direction.                */
...
* DESCRIPTION
*
*       Write data by SCSI read command.
*
* INPUTS
*
*       handle                              Handle.
...
*
**************************************************************************/
STATUS  NU_USBH_MS_SCSI_Write10(VOID    *handle,
                                UINT8   command[NU_SCSI_CML_MAX],
                                UINT32  block_addr,
                                UINT32  block_num,
                                VOID    *buffer,
                                UINT32  buf_len )
{
    ...
    memset(command, 0, NU_SCSI_CML_WRITE10);

    /* Set command code. */
    command[0] = NU_SCSI_CMD_WRITE10;
    ...
    status = NU_USBH_MS_SCSI_Request(
                    handle,             /* Handle.                   */
                    command,            /* Pointer to command block. */
                    NU_SCSI_CML_WRITE10,/* Length of CBWCB.          */
                    buffer,             /* Pointer to buffer.        */
                    buf_len,            /* Data length.              */
                    USB_DIR_OUT);       /* Direction.                */
...
* DESCRIPTION
*
*       Request to transfer a SCSI command.
*
* INPUTS
*
*       handle                              Handle.
...
*
**************************************************************************/
STATUS  NU_USBH_MS_SCSI_Request(VOID    *handle,
                                VOID    *command,
                                UINT32  cmd_len,
                                VOID    *buffer,
                                UINT32  buf_len,
