...
* FUNCTIONS
*
*       StoreFat                            Store fat onto flash device.
*       GetFat                              Gets last stored fat back
*                                           from flash.
*       StoreSector                         Writes a sector to flash.
*       GetSector                           Reads a sector from flash.
*       Format                              Format the volume.
*       fs_mount_dataflashdrive             Mount function for this driver.
*       fs_getmem_dataflashdrive            Returns required memory
*                                           in bytes for this driver.
*       
*
*************************************************************************/
...
long a;
	...
	if (vi->version) *vi->version=F_DRVVERSION;
	...
	for (a=0; a<(long)((unsigned long)vi->maxfat>>1); a++) {
...
unsigned short freeblock;
	...
	{
		if (vi->fatbits[(unsigned long)relsector>>5]&(1UL<<((unsigned long)relsector&31))) 
		{
...
#else
		{
			...
			if (fsm_checksectorfree(vi,sector)) {
				...
				for (c=0; c<vi->flash->sectorperblock; c++) { /* copy used sectors */
					if (!fsm_checksectorfree(vi,vi->flash->sectorperblock*a+c)) { /* check if data there */
						usedbefore=1;
						vi->fatbits[(unsigned long)c>>5]&=~(1UL<<((unsigned long)c&31)); /* clear erase bits */
						...
						vi->fatbits[(unsigned long)c>>5]|=(1UL<<((unsigned long)c&31)); /* set erase bits */
					}
				}

				vi->fatbits[(unsigned long)b>>5]&=~(1UL<<((unsigned long)b&31)); /* clear erase bits */
 ...
 * INPUTS
 *
 * vd - volume descriptor of the volume
 *
 * RETURNS
 *
...
int fs_mount_dataflashdrive(FS_VOLUMEDESC *vd,FS_PHYGETID phyfunc) {
...
long fs_getmem_dataflashdrive(FS_PHYGETID phyfunc) {
FS_FLASH flash;
long mem=fsm_allocdatasize(sizeof(FS_VOLUMEINFO));     /* volumeinfo */

	fsm_memset(&flash,0,sizeof(FS_FLASH)); /* reset entries */

	if (phyfunc(&flash)) return 0; /* get flash properties, if not identified return zero */
	flash.descsize-=flash.cacheddescsize;

	mem+=fsm_allocdatasize(sizeof(FS_FLASH));          /*  flash prop  */

	mem+=fsm_allocdatasize(sizeof(FS_FILEINT)*MAXFILE); /*  maximum number of files  */
